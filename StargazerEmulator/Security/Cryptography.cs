...
/**
 * Class written by Quackster based on decompiled v28 fuse_client from ProjectorRays.
 * Copyright: 2023
 *
 * <3
 */
public class Cryptography {
    ...
    private Dictionary<int, int> pTableList = new Dictionary<int,int>();

    public Cryptography(byte[] tMyKey) {
        ...
        for (int k = 0; k < this.pTableStr.Length; k++) {
            this.pTableList.Add((int) this.pTableStr[k], k);
        }
        ...
        int l = 0;
        foreach (int value in tMyKey) {
            int tVal = value ^ tXorVals[l];
            tModKey.Add(tVal);
            l = (l + 1) % tXorVals.Length;
        }
        for (int q = 0; q < 256; q++) {
            tKey[q] = tModKey[q % tModKey.Count];
            pR3hu24v5[q] = q;
        }
    ...
    public String AkwGx8bHG2kc1xGG4xbdHPCV0fqvK(String tdata)
    {
        ...
        for (var a = 1; a <= tDataOut.Length;) {
            ...
            tCipher.Append(pTableStr[tByte1]).Append(pTableStr[tByte2]);

            if (tDataOut.Length > a) {
                int tByte3a = (tNum2 & 15) * 4;
                int tByte3b = (tNum3 & 192) / 64;
                int tByte3 = tByte3a | tByte3b;
                tCipher.Append(pTableStr[tByte3]);
            }

            if (tDataOut.Length > a + 1) {
                int tByte4 = tNum3 & 63;
                tCipher.Append(pTableStr[tByte4]);
            }
            a += 3;
        }
        Random rand = new Random();
        int i = rand.Next(256) - 1;
    ...
    public String kg4R6Jo5xjlqtFGs1klMrK4ZTzb3R(String tdata) {
        ...
        while (a < tdata.Length) {
            ...
            int tNum1 = pTableList[((int)tdata[a])];
            int tNum2 = pTableList[((int)tdata[a + 1])];
            if (tNum2 < 0) {
                tNum2 = 0;
            }
            int tByte1a = tNum1 * 4;
            ...
            int tNum3 = 0;
            if (tdata.Length > (a + 2)) {
                tNum3 = pTableList[((int)tdata[a + 2])];
                if (tNum3 < 0) {
                    tNum3 = 0;
                }
            ...
            if (tdata.Length > (a + 3)) {
                int tNum4 = pTableList[((int)tdata[a + 3])];
                if (tNum4 < 0) {
                    tNum4 = 0;
                }
